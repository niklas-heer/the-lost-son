...
export default class extends Phaser.State {
  ...
  fadeout () {
    this.game.add.tween(this.pic.scale).to({x: 3, y: 3}, this.duration + 1000, "Linear", true, 0, 0)
    this.game.add.tween(this.pic).to({alpha: 0}, this.duration, "Linear", true, 0, 0)
  ...
  create () {
    ...
    this.sound.play()
    this.game.add.tween(this.pic.scale).to({x: 0.02, y: 0.02}, this.duration, "Linear", true, 0, 0)
